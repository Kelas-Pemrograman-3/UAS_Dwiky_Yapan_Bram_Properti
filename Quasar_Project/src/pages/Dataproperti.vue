    ...
    >
      ...
      <template v-slot:body="props">
          <q-tr :props="props">
            ...
            <q-td key="Fasilitas" :props="props">{{ props.row.Fasilitas }}</q-td>
   ...
   <q-dialog v-model="openedit" v-if="openedit">
      <q-card style="width: 700px; max-width:80vw;">
        ...
        <q-separator/>
          ...
          <q-input label ="Tipe" v-model ="activedata.Tipe"></q-input>
          <q-input label ="Ukuran" v-model ="activedata.Ukuran"></q-input>
          <q-input label ="Harga" v-model ="activedata.Harga"></q-input>
          <q-input label ="Alamat" v-model ="activedata.Alamat"></q-input>
          <q-input label ="Fasilitas" v-model ="activedata.Fasilitas"></q-input>
          </q-card-selection>
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        { name: 'Tipe', align: 'center', label: 'Tipe', field: 'Tipe', sortable: true },
        { name: 'Ukuran', label: 'Ukuran', field: 'Ukuran', sortable: true },
        { name: 'Harga', label: 'Harga', field: 'Harga', sortable: true },
        { name: 'Alamat', label: 'Alamat', field: 'Alamat', sortable: true },
        { name: 'Fasilitas', label: 'Fasilitas', field: 'Fasilitas', sortable: true },
  ...
  methods: {
    getData () {
      this.$axios.get('DataProperti/getAll')
        .then(res => {
    ...
    edit () {
      this.$axios.put('DataProperti/edit/' + this.activedata._id, this.activedata)
        .then(res => {
          if (res.data.error) {
    ...
    confirm (id) {
      this.$q.dialog({
        ...
        this.$axios.delete('DataProperti/delete/' + id)
          .then(res => {
            if (res.data.error) {
              this.showNotif(res.data.pesan, 'negative')
            } else {
